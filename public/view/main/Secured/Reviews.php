...
include($_SERVER['DOCUMENT_ROOT'] . '/comp353-project/config/dbMakeConnection.php');
            ...
            if ($status == 1) {
                ...
                // GET Rider rides
                $stmt = $d->conn->prepare("SELECT Ride.RideId from ".$GLOBALS['db_name'].".Ride join ".$GLOBALS['db_name'].".Rider on Ride.RideId = RiderId where Rider.RiderId =:u");
                $stmt->bindParam(':u', $u);
                $stmt->execute();
                ...
                // GET Driver rides
                $stmt = $d->conn->prepare("SELECT Ride.RideId from ".$GLOBALS['db_name'].".Ride join ".$GLOBALS['db_name'].".Driver on Ride.RideId=DriverId where DriverId =:u");
                $stmt->bindParam(':u', $u);
                $stmt->execute();
                $result = $stmt->fetchAll();
