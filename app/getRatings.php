...
$u = $_SESSION['username'];
$stmt = $d->conn->prepare("SELECT UserId FROM ".$GLOBALS['db_name'].".Member WHERE UName = :u");
$stmt->bindParam(':u', $u);
$stmt->execute();
...
$stmt = $d->conn->prepare("SELECT max(Score) as High, min(Score) as Low, avg(Score) as Avg FROM ".$GLOBALS['db_name'].".Ratings WHERE RateeId = :r");
